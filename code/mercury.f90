...
! Modified by Beibei Liu (Nov 2017): 
! added or changed file: user_module.f90, massgrowth.f90, algo_hybrid.f90, dynamics.f90,
! system_properties.f90, mercury_output.f90, mercury_global.f90,
! messgin.in, param.in, big.in  
!1. add disk parameter into the MODULE mercury_gloabl.f90 and read it from the param.in; add these new constants in mercury.f90
!siggas0, hgas0, tau_dep, index_p, index_q, Mdot_peb, tau_s,alpha, opt(9)
!1). mercury_output.f90: add opflag=0,siggas0... save the output information
...
!4. mercury.f90: include the pebble accretion in B-S method and  hybrid method when opt(9)=1 
!4. add planetary compostion (element X): m_x for the following subrountine
! initial setup: big.in  mio_in 
! main integration: mal_hvar, mal_con, massgrowth, mxx_elim, mce_coll, mce_init, mce_merg, mdt_hkce, mdt_hy 
! write output : mio_dump, element.f90
!5. collision accounts for the density change: mce_coll, mce_merg
!------------------------------------------------------------------------------
...
program mercury
  ...
  real(double_precision), dimension(:), allocatable :: m,m_x,rho,rceh,epoch ! (Number of bodies)
...
!~ 
  ...
  allocate(m_x(nb_bodies_initial), stat=error)
  if (error.ne.0) then
    write(*,*) 'Error: failed to allocate "m_x" array'
  end if
  m_x(1:nb_bodies_initial) = 0.d0
...
subroutine mio_in (time,h0,tol,rcen,jcen,en,am,cefac,ndump,nfun,nbod,nbig,m,x,v,s,rho,rceh,&
     id,epoch,ngf,opflag,ngflag,m_x)
...
!!\n                              3 = both
  ...
  real(double_precision), intent(out) :: m_x(nb_bodies_initial) !< [out] mass of X element (in solar masses * K2)
  ...
  do j = 1, 40
     ...
     if (j.eq.27) read (c80,*,err=661) alpha_t
     if (j.eq.28) read (c80,*,err=661) alpha
     if (j.eq.29) read (c80,*,err=661) mdot_gas0
     if (j.eq.30) read (c80,*,err=661) L_s
     if (j.eq.31) read (c80,*,err=661) Rdisk0
...
!  print*,k
        ...
        else if (c80(1:1).eq.'x'.or.c80(1:1).eq.'X') then
           m_x(nbod) = temp * K2
     ...
     if (informat.eq.1) then
        read (11,*,err=666) x(1,nbod),x(2,nbod),x(3,nbod),v(1,nbod),v(2,nbod),v(3,nbod),s(1,nbod),s(2,nbod)
     else
...
subroutine mal_hvar (time,h0,tol,jcen,rcen,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,&
     id,ngf,opflag,ngflag,onestep,m_x)
...
!!\n                              1 = main integration, full output
  ...
  real(double_precision), intent(inout) :: m_x(nbod) !< [in,out] mass of element X (in solar masses * K2)
  ...
  ! Local
  integer :: i,j,k,n,itmp,nhit,ihit(CMAX),jhit(CMAX),chit(CMAX),im
  ...
  call mce_init (h0,jcen,rcen,cefac,nbod,nbig,m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,outfile(2),1,m_x)
  ...
  do
     ...
     if (abs(tout-time).lt.abs(tsmall).and.opflag.ge.-1) then
        ...
        call mio_out (time,jcen,rcen,nbod,nbig,m,xh,vh,s,rho,stat,id,opflag,outfile(1),m_x)
...
!~      v0(:,:) = vh(:,:)
     ...
     time = time + hdid
     ! pebble accretion for B-S method    
     if (opt(9) == 1) then
     ...
     if (nhit.gt.0.and.opt(2).ne.0) then
        do k = 1, nhit
           if (chit(k).eq.1) then
              i = ihit(k)
              j = jhit(k)
!        print*,'before time ',time/365.25d0
!        print*,'before m ', time,m/K2,m_x/K2
!        print*,'before r ', rphys
!        print*,'before d ', rho/498.
!        print*,'before rce', rce
              call mce_coll (thit(k),en(3),jcen,i,j,nbod,nbig,m,xh,vh,s,rphys,stat,id,outfile(3),m_x,rho)
!        print*,'after time ',time/365.25d0
!        print*,'after m ', m/K2,m_x/K2
!        print*,'after r ', rphys
!        print*,'after d ', rho/498.
!        print*,'after rce', rce
           end if
        end do
        ! Remove lost objects, reset flags and recompute Hill and physical radii
        call mxx_elim (nbod,nbig,m,xh,vh,s,rho,rceh,rcrit,ngf,stat,id,outfile(3),itmp,m_x)
        dtflag = 1
        if (opflag.ge.0) opflag = 1
        ! at right now only change the mass and density of the planet, not change the radius, radius is calculated in mce_init
        call mce_init (h0,jcen,rcen,cefac,nbod,nbig,m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,outfile(2),1,m_x)
     end if
     ...
     if (nhit.gt.0) then
        ...
        dtflag = 1
        if (opflag.ge.0) opflag = 1
        call mce_init (h0,jcen,rcen,cefac,nbod,nbig,m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,outfile(2),0,m_x)
     end if
...
subroutine mal_hcon (time,h0,tol,jcen,rcen,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,&
     id,ngf,opflag,ngflag,onestep,coord,bcoord,m_x)
...
!!\n                              1 = main integration, full output
  ...
  ! Local
  integer :: i,j,k,n,itmp,nclo,nhit,jhit(CMAX),iclo(CMAX),jclo(CMAX),im
  ...
  do
     ...
     if (abs(tout-time).le.hby2.and.opflag.ge.-1) then
        ...
        call mio_dump (time,h0,tol,jcen,rcen,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,id,ngf,epoch,opflag,m_x)
        tdump = time
     end if
