...
program element
  ...
  logical test
  character(len=250) :: string,fout,header,infile(5000)
  character(len=80) :: cc
  character(len=5) :: fin
...
!!\n                              3 = both
  ...
  real(double_precision), dimension(:), allocatable :: m, m_x, a, ns, is ! (Number of bodies)
...
40   read (10,'(a250)') string
     ...
     inquire (file=infile(j), exist=test) ! xv*.out file
...
100  continue
     ...
     if (type.eq.'a') then
        ...
        do j = 1, nbig + nsml
           ...
           id(k) = c(j)(4:11)
           el(18,k) = mio_c2fl (c(j)(12:19))!m
           el(22,k) = mio_c2fl (c(j)(52:59))!m_x
...
900  continue
  ...
  if ((timestyle.eq.0).or.(timestyle.eq.2)) then
      !modified by CWO
     write (10,'(a,es25.16)') '#pars::[Time(days)]::',t1
  else if (timestyle.eq.1) then
     write (10,'(/,a,i10,1x,i2,1x,f8.5,/)') ' Date: ',year,month,t1
  else if (timestyle.eq.3) then
      !modified by CWO
     write (10,'(a,es25.16)') '#pars::[Time(years)]::',t1
  end if
  !modified by CWO
  write (10,'(2a)') '#cols::[id,mass,m_x,sma,ecc,inc,peri,node,M,x,y,z,vx,vy,vz,rot/day,Obl]',''
  ...
  do j = 1, nbod
     k = code(iback(j))
     write (10,213) id(k),el(18,k),el(22,k),&
            el(1,k),el(2,k),el(3,k),el(4,k),el(5,k),el(6,k),&
            el(10,k),el(11,k),el(12,k),el(13,k),el(14,k),el(15,k),&
            el(20,k), el(19,k)
  end do

  !------------------------------------------------------------------------------

  ! Format statements (changed by CWO)
213 format (1x,a8,1x,14es24.16,1p,e11.4,f9.3,1x,f9.2)
!213 format (1x,a8,1x,es22.15,1x,f7.5,1x,f7.3,1p,e11.4,0p,1x,f6.3,1x,f6.2)
