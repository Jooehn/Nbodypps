...
subroutine mdt_hy (time,h0,tol,en,am,jcen,rcen,nbod,nbig,m,x,v,s,rphys,rcrit,rce,stat,id,ngf,dtflag,ngflag,&
     opflag,colflag,nclo,iclo,jclo,dclo,tclo,ixvclo,jxvclo,m_x,rho)
...
!!\n                              1 = main integration, full output
  ...
  real(double_precision), intent(inout) :: rho(nbod) ! density
  ...
  if (dtflag.ne.2) then
     ...
     if (opt(8).eq.1) call mfo_user (time,jcen,nbod,nbig,m,rphys,x,v,ausr)
...
!~   v0(:,:) = v(:,:)
  ...
  if (nce.gt.0) then
     ...
     call mdt_hkce (time,h0,hrec,tol,en(3),jcen,rcen,nbod,nbig,m,x,v,s,rphys,rcrit,rce,stat,id,ngf,ngflag,&
          colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,ixvclo,jxvclo,mfo_hkce,m_x,rho)
  end if
...
subroutine mdt_hkce (time,h0,hrec,tol,elost,jcen,rcen,nbod,nbig,m,x,v,s,rphy,rcrit,rce,stat,id,ngf,ngflag,&
     colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,ixvclo,jxvclo,force,m_x,rho)
...
!!\n                              3 = both
  ...
  real(double_precision), intent(inout) :: m(nbod) !< [in,out] mass (in solar masses * K2)
  real(double_precision), intent(inout) :: m_x(nbod) !< [in,out] mass of element X (in solar masses * K2)
  real(double_precision), intent(inout) :: rho(nbod) !< [in,out] density
  real(double_precision), intent(inout) :: x(3,nbod)
  real(double_precision), intent(inout) :: v(3,nbod)
  real(double_precision), intent(inout) :: s(3,nbod) !< [in,out] spin angular momentum (solar masses AU^2/day)
  ...
  if ((nhit.gt.0).and.(opt(2).ne.0)) then
     do k = 1, nhit
        if (chit(k).eq.1) then
           i = ihit(k)
           j = jhit(k)
           call mce_coll (thit(k),elost,jcen,i,j,nbs,nbsbig,mbs,xbs,vbs,sbs,rphybs,statbs,idbs,outfile(3),m_x,rho)
           colflag = colflag + 1
        end if
     end do
  end if
