...
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
subroutine mio_dump (time,h0,tol,jcen,rcen,en,am,cefac,ndump,nfun,nbod,nbig,m,x,v,s,rho,rceh,stat,id,ngf,epoch,opflag,m_x)
...
!!\n                              1 = main integration, full output
  ...
  real(double_precision), intent(in) :: m(nbod) !< [in] mass (in solar masses * K2)
  real(double_precision), intent(in) :: m_x(nbod) !< [in] mass of element X (in solar masses * K2)
  ...
  do idp = 1, 2
     ...
     do i = 1, 2
        ...
        do j = j1, j2
           ...
           if (m(j).gt.0) then
              write (c(len+1:len+25),'(1p,a3,e22.15)') ' m=',m(j)*k_2
              len = len + 25
              write (c(len+1:len+25),'(1p,a3,e22.15)') ' x=',m_x(j)*k_2
              len = len + 25
           end if
           do k = 1, 3
...
45   if (idp.eq.2) open (33, file=dumpfile(3), status='old', err=45)
     ...
     write (33,*) mem(188)(1:lmem(188)),alpha_t
     write (33,*) mem(189)(1:lmem(189)),alpha
     write (33,*) mem(190)(1:lmem(190)),mdot_gas0
     write (33,*) mem(191)(1:lmem(191)),L_s
     write (33,*) mem(192)(1:lmem(192)),Rdisk0
...
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
subroutine mio_out (time,jcen,rcen,nbod,nbig,m,xh,vh,s,rho,stat,id,opflag,outfile,m_x)
  ...
  real(double_precision), intent(in) :: rcen !< [in] radius of central body (AU)
  real(double_precision), intent(in) :: m(nbod) !< [in] mass (in solar masses * K2)
  real(double_precision), intent(in) :: m_x(nbod) !< [in] mass (in solar masses * K2)
...
!!\n                              1 = main integration, full output
  ...
  character(len=180) :: header,c(nb_bodies_initial)
  ...
  if (opflag.eq.-1.or.opflag.eq.1 .or. opflag.eq.0) then
